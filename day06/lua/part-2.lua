...
local function solveSingleFish(timer, limit)
    ...
    if savedResults[timer] ~= nil and savedResults[timer][limit] ~= nil then
        return savedResults[timer][limit]
    end

    local newLimit = limit - timer
    local result = solveSingleFish(7, newLimit) + solveSingleFish(9, newLimit)
    if savedResults[timer] == nil then savedResults[timer] = {} end
    savedResults[timer][limit] = result
    return result
end
...
local function solveTask(fileIterator)
    ...
    for _, fish in pairs(fishes) do
        fishCount = fishCount + solveSingleFish(fish, 256)
    end
