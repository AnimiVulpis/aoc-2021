...
local function play_round(hit, boards, skip_check)
    for index, board in pairs(boards) do
        ...
        if skip_check ~= true then
            local winner = check_win(board)
            if winner == true then return true, {board} end
        end
    end
...
local function play(hit_list, board_list)
    for index, hit in pairs(hit_list) do
        local winner, boards = play_round(hit, board_list, index <= 5)
        if winner == true then return hit, boards[1] end
    end
    -- Should not happen
    return nil
end
